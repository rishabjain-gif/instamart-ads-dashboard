        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-800 text-white">
              ...
              {hasPrevMonth && <th className="px-2 py-3 text-center font-semibold text-xs">ROAS Δ%</th>}
              <th className="px-3 py-3 text-center font-semibold">CPC (₹)</th>
              {hasPrevMonth && <th className="px-2 py-3 text-center font-semibold text-xs text-orange-300">CPC Δ%<br/><span className="font-normal opacity-70">(↑ bad)</span></th>}
              <th className="px-3 py-3 text-center font-semibold">CVR %</th>
              {hasPrevMonth && <th className="px-2 py-3 text-center font-semibold text-xs">CVR Δ%<br/><span className="font-normal opacity-70">(↓ bad)</span></th>}
            </tr>
          </thead>
          <tbody>
            {categories.map(cat => {
              ...
              return (
                <Fragment key={cat}>
                  <tr className="bg-gray-800 text-white cursor-pointer hover:bg-gray-700" onClick={() => toggleCat(cat)}>
                    <td className="px-4 py-2.5 font-bold"><span className="text-gray-400 text-xs mr-2">{isCatExp ? '▼' : '▶'}</span>{cat}</td>
                    <td className="px-3 py-2.5 text-right font-bold">{fmtSpend(catTotalSpend)}</td>
                    <td colSpan={colCount - 2} className="px-3 py-2.5 text-center text-gray-400 text-xs italic">{campNames.length} campaign{campNames.length !== 1 ? 's' : ''}</td>
                  </tr>
                    ...
                    return (
                      <Fragment key={campKey}>
                        <tr className="bg-gray-100 cursor-pointer hover:bg-gray-200" onClick={() => toggleCamp(campKey)}>
                          ...
                          <td colSpan={colCount - 2} className="px-3 py-2 text-center text-gray-400 text-xs italic">{kwRows.length} keyword{kwRows.length !== 1 ? 's' : ''}</td>
                        </tr>
