...
function CategoryLabel({ name }) {
  if (name === 'Uncategorised') return <span className="text-amber-600">{name}</span>;
  return <>{name}</>;
}
...
export default function MonthlyRoas({ platform = 'instamart' }) {
  ...
  const hasMissingCat = (data.missingCategoryCount || 0) > 0;
  const hasMissingKw = (data.missingKeywordCount || 0) > 0;
    ...
    <div className="mb-4 flex items-start gap-3 flex-wrap">
      ...
      {hasDataIssues && (
        <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 border border-amber-300 rounded-lg text-xs text-amber-800">
          <span>⚠️</span>
          <span>
            {hasMissingCat && <span><strong>{data.missingCategoryCount}</strong> rows have no category — shown as <em>Uncategorised</em> below</span>}
                ...
                <Fragment key={cat}>
                  <tr className="bg-gray-100 cursor-pointer hover:bg-gray-200 transition-colors" onClick={() => toggleCat(cat)}>
                    <td className="px-4 py-2.5 font-semibold text-gray-800">
                      <span className="text-gray-400 text-xs mr-2">{isExpanded ? '▼' : '▶'}</span><CategoryLabel name={cat} />
                    </td>
