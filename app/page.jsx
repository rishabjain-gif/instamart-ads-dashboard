...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (p === 'zepto' || p === 'instamart') setPlatform(p);
  }, []);
            ...
            <div className="flex items-center bg-gray-800 rounded-lg p-1 gap-1">
              <button
                ...
                className={'px-4 py-1.5 rounded-md text-sm font-semibold transition-all ' +
                  (platform === 'instamart' ? 'bg-orange-500 text-white shadow' : 'text-gray-400 hover:text-white')}>
                Instamart
              </button>
              <button
                onClick={() => handleSetPlatform('zepto')}
                className={'px-4 py-1.5 rounded-md text-sm font-semibold transition-all ' +
                  (platform === 'zepto' ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white')}>
                Zepto
              </button>
            </div>
          ...
          {TABS.map(tab => (
            <button
              ...
              className={'px-5 py-3.5 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ' +
                (activeTab === tab.id
                  ? (platform === 'zepto' ? 'border-purple-600 text-purple-600' : 'border-blue-600 text-blue-600')
