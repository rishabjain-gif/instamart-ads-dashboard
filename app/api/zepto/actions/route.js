...
function aggRows(rows) {
  let spend = 0, gmv = 0, clicks = 0, orders = 0;
  for (const r of rows) {
    spend += toNum(r['Spend']);
...
export async function GET() {
  try {
    ...
    const cacheKey = 'act_z_' + current.key + '_' + (previous ? previous.key : 'none');
    ...
    // Sort available dates (DD/MM/YYYY format — sort by parsed value)
    const parseZDate = (s) => {
      if (!s) return 0;
      const p = s.split('/');
    ...
    const allDates = [...new Set(currRows.map(r => r['Date']).filter(Boolean))].sort((a,b) => parseZDate(a)-parseZDate(b));
    ...
    for (const row of currRows) {
      ...
      const k = row['KeywordName'] || 'Unknown';
      const cat = row['Category'] || 'Unknown';
      if (!k || k === 'Unknown') continue; // skip non-keyword rows
    ...
    for (const [campName, campData] of Object.entries(camps)) {
      const last3Spend = campData.allRows
        .filter(r => last3Dates.has(r['Date']))
        .reduce((s, r) => s + toNum(r['Spend']), 0);
      ...
      if (canSplit) {
        const last7 = aggRows(campData.allRows.filter(r => last7Dates.has(r['Date'])));
        const earlier = aggRows(campData.allRows.filter(r => earlierDates.has(r['Date'])));
