...
function buildInsightText(roasChg, cpcChg, cvrChg, spendChg) {
  ...
  if (cpcChg !== null && cpcChg > 10) { reasons.push('CPC rose ' + cpcChg.toFixed(0) + '%'); actions.push('review high-CPC keywords and consider reducing bids'); }
  if (cvrChg !== null && cvrChg < -10) { reasons.push('CVR dropped ' + Math.abs(cvrChg).toFixed(0) + '%'); actions.push('check product page quality, pricing and stock availability'); }
  if (spendChg > 20 && cpcChg !== null && cpcChg > 0) { reasons.push('spend scaled ' + spendChg.toFixed(0) + '%'); actions.push('reduce budget on keywords with ROAS below target before scaling further'); }
  if (reasons.length === 0) { actions.push('audit individual keyword ROAS below and pause under-performers'); }
  return { reason: reasons.length ? reasons.join(', ') : 'general ROAS decline', action: actions.join('; ') };
}
...
export async function GET(request) {
  try {
    ...
    const catTotals = {}; const campTotals = {}; const table = [];
    ...
    for (const [k, g] of Object.entries(currGroups)) {
      ...
      table.push({
        ...
        spend: agg.spend, roas: agg.roas > 0 ? agg.roas : null,
        cpc: agg.cpc > 0 ? agg.cpc : null, cvr: agg.cvr > 0 ? agg.cvr : null,
    ...
    table.sort((a, b) => {
      const cd = (catTotals[b.category] || 0) - (catTotals[a.category] || 0); if (cd !== 0) return cd;
      const ckA = a.category + '|||' + a.campaign; const ckB = b.category + '|||' + b.campaign;
      const campD = (campTotals[ckB] || 0) - (campTotals[ckA] || 0); if (campD !== 0) return campD;
      return b.spend - a.spend;
    });

    const campAggs = {}; const campPrevAggs = {};
    for (const [k, g] of Object.entries(currGroups)) {
      const parts = k.split('|||'); const category = parts[0], campaign = parts[1];
      const ck = category + '|||' + campaign;
    ...
    for (const [ck, cd] of Object.entries(campAggs)) {
      const parts = ck.split('|||'); const category = parts[0], campaign = parts[1];
      const curr = aggregateZeptoRows(cd.rows);
      const prevData = campPrevAggs[ck];
      const prev = prevData ? aggregateZeptoRows(prevData.rows) : null;
      ...
      insights.push({ category, campaign, spend: curr.spend, prevSpend: prev.spend, spendChange: spendChg,
        roas: curr.roas, prevRoas: prev.roas, roasChange: roasChg,
        cpc: curr.cpc, prevCpc: prev.cpc, cpcChange: cpcChg,
        cvr: curr.cvr, prevCvr: prev.cvr, cvrChange: cvrChg, reason, action });
    }
    ...
    const result = { monthLabel: sheet.label, monthKey: key, prevMonthLabel: prevSheet ? prevSheet.label : null, data: table, insights, availableMonths };
